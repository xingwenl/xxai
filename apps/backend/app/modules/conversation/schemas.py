...
_BLOCK_TYPES = {"text", "markdown", "image", "file", "table", "chart", "actions", "custom", "error"}


def sanitize_content_blocks(blocks: list[dict[str, Any]] | None) -> list[dict[str, Any]]:
    ...
    for index, raw in enumerate(blocks[:32]):
        if not isinstance(raw, dict):
            safe.append({"id": f"invalid_{index}", "type": "error", "text": "内容块格式无效", "status": "failed"})
            continue
        block = dict(raw)
        block["id"] = str(block.get("id") or f"block_{index}")[:128]
        block_type = str(block.get("type") or "error")
        if block_type not in _BLOCK_TYPES:
            block = {"id": block["id"], "type": "error", "text": "暂不支持的内容类型", "status": "failed"}
        elif block_type in {"text", "markdown", "error"}:
            block["text"] = str(block.get("text") or block.get("fallback") or "")[:100_000]
            ...
            if asset_id is not None and not _ASSET_ID.fullmatch(str(asset_id)):
                block = {"id": block["id"], "type": "error", "text": "资源标识无效", "status": "failed"}
            elif asset_id is not None:
                block["asset_id"] = str(asset_id)
            elif not str(block.get("url") or "").startswith(("https://", "http://")):
                block = {"id": block["id"], "type": "error", "text": "资源地址无效", "status": "failed"}
            ...
            if not _CUSTOM_NAME.fullmatch(name) or not fallback:
                block = {"id": block["id"], "type": "error", "text": "自定义组件不可用", "status": "failed"}
            else:
                block["component_name"] = name
                block["fallback"] = fallback
                block["props"] = block.get("props") if isinstance(block.get("props"), dict) else {}
