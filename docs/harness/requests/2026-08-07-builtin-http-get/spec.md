...
- 在智能体管理页提供内置工具管理入口，使管理员无需直接调用 API 即可查看并启停工具。
- 在前端 SDK 中将 `builtin_tool` 作为正式 Agent Loop 步骤类型，并按普通工具展示。
...
- 智能体列表每行增加内置工具管理入口，沿用现有图标按钮、Tooltip 和 Dialog 交互。
- 管理弹窗打开后读取该智能体的完整内置工具目录与启用状态；每项展示名称、说明、副作用等级和启用开关。
- 开关变更立即调用现有绑定更新 API。提交期间只禁用当前工具；成功后更新缓存并提示结果，失败时保留原状态并展示错误。
- 加载过程展示占位状态；加载失败提供重试操作；目录为空时展示空状态。
- 前端 SDK 的 `AgentLoopStepType` 增加 `builtin_tool`，过程面板与摘要逻辑将其归类为“调用工具”，工具步骤详情沿用 `host_tool`、`mcp_tool` 的展示行为。
- 本次不新增或修改后端 API、数据库模型、权限规则和 Agent Loop 事件字段。
...
- 智能体管理页可加载当前 Agent 的内置工具状态，并能启用或停用 `http_get`；加载、空列表、失败与保存中状态均有明确反馈。
- 前端 SDK 类型定义、过程面板和摘要均将 `builtin_tool` 识别为工具步骤，不再错误显示为普通思考步骤。
- 数据库迁移、模型约束、服务测试、路由测试、运行时测试、Ruff、Black 和相关回归测试通过。
- 管理端类型检查/构建与 SDK 单元测试通过。
- `verify.md` 记录真实命令、输出、失败项和生产网络出口未在单元测试中覆盖的剩余风险。

## 变更记录

### 2026-08-07 第 1 次增量变更

- 变更原因：后端内置工具接口完成后，需要在智能体管理页提供可操作入口；同时发现 SDK 尚未正式识别 `builtin_tool` Agent Loop 步骤。
- 变更内容：新增智能体内置工具管理弹窗、前端 API 封装与启停交互；补齐 SDK 步骤类型、工具分类展示和相关测试。
- 影响章节：目标、管理端与 SDK 接入、工具调用链与工程验证。
- 是否触发人工确认：否；复用现有 API 与事件值，不改变架构、数据模型、API 契约或权限语义。
