...
- `apps/backend/app/modules/conversation/models.py` 已有独立 `model_usage_records` 表，记录平台、Agent、Agent 版本、Embed Client、最终用户、会话、请求和三类 token。
- `apps/backend/app/modules/platform/router.py`、Embed Client API 和前端 `apps/front/src/api/*` 已形成平台管理员读取与 React Query 刷新模式。
- 当前没有用量查询 API 和后台页面，不能直接从前端读取已有明细。

### 方案比较

| 方案 | 优点 | 限制 | 决策 |
|---|---|---|---|
| 单页汇总 + 明细 | 筛选一次同时刷新统计和明细，入口少，适合运营排查 | 页面信息密度较高 | 采用 |
| 汇总页与明细页拆开 | 单页更轻 | 入口增加，筛选状态需要跨页同步 | 不采用 |
| 只做明细接口和表格 | 开发量最小 | 无法快速判断 Agent、Client 和日期趋势 | 不采用 |

### 最终决策

- 复用当前 Phase 2D 管理后台，新增 `/ai/model-usage` 单页。
- 后端新增分页明细接口和汇总接口，两个接口共享平台、Agent、Client、日期范围筛选。
- 汇总接口一次返回总计、按 Agent、按 Client、按日期三种聚合结果。
- 初版不新增汇总表、不引入图表库、不展示 prompt、回复、token 或密钥。
- 用户已于 2026-07-30 确认采用单页汇总 + 明细方案，并要求加入按 Agent / Client / 日期统计。
