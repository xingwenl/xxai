...
- 变更原因：将 SDK 使用方法和生产接入方法集中到 `apps/ai-sdk/README.md`，降低第三方接入时在多个文档间跳转的成本。
- 变更内容：补充接入前准备、Token 方式选择、服务端代理完整示例、本地 Demo、无后端代理的安全边界、生命周期、UI、工具、常见问题和安全清单；明确当前版本不支持安全的纯浏览器生产换 token。
- 影响章节：范围、验收标准、风险。
- 是否触发人工确认：否，仅文档整理，不改变运行时行为、API 契约或鉴权语义。
- 关联计划更新：沿用已完成实现，补充 README 文档校验。
